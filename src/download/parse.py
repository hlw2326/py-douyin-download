...
from ..config import Settings, Account, Colors
...
class Parse:
    ...
    def _extract_video( video: dict, result: dict):
        ...
        # duration 单位为毫秒，转为秒（向上取整避免边界值误差）
        duration_ms = Parse._extract_value(video, 'duration') or 0
        result['duration'] = (int(duration_ms) + 999) // 1000
        ...
        for item in items:
            ...
            if (result['create_time_date'] <= latest) and (result['create_time_date'] >= earliest):
                if (gallery := Parse._extract_value(item, 'images')):
                    ...
                    if settings.max_video_duration > 0 and result['duration'] > settings.max_video_duration:
                        print(f'[{Colors.YELLOW}]跳过超长视频 {result["id"]} ({result["duration"]}s > {settings.max_video_duration}s): {result["desc"][:30]}')
                        continue
                    results.append(result)
        return results
